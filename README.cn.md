# xonsh

[English version](./README.md)

🐚 Python-powered shell. Full-featured, cross-platform and AI-friendly.

[![x-cmd/install — xonsh Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/xonsh.svg?lang=zh)](https://x-cmd.com/install/xonsh)

## 安装

```sh
x install xonsh
```

## 代码洞察

合计: **133,380** 行代码（覆盖前 5 种语言、共 **442** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Python | 90,074 | 7,726 | 18,038 | 340 |
| Css | 23,617 | 467 | 2,968 | 23 |
| ReStructuredText | 9,579 | 0 | 3,599 | 53 |
| Svg | 8,697 | 5 | 2 | 6 |
| JavaScript | 396 | 154 | 75 | 20 |

## OpenSSF Scorecard 评分

总评分: **4.5 / 10**

评分最低的几项:

- **Code-Review** (4/10) — Found 6/14 approved changesets -- score normalized to 4
- **Token-Permissions** (0/10) — detected GitHub workflow tokens with excessive permissions
- **CII-Best-Practices** (0/10) — no effort to earn an OpenSSF best practices badge detected

## 源代码

- **上游仓库**: <https://github.com/xonsh/xonsh>
- **官网**: <http://xon.sh>
- **许可证**: NOASSERTION

## 发布

- **最新版本**: `0.24.2` (2026-08-23)
- **最近提交**: 2026-09-22
- **Release 含资产**: 2 个

## 流行度

- **Star**: 9,652 · **Fork**: 743 · **开放 issue**: 2,853 · **贡献者**: 351

## 累计统计

- **发布数**: 146 · **已合并 PR**: 3068 · **开放 PR**: 9 · **已关闭 issue**: 2787 · **开放 issue**: 66 · **提交数**: 11973

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-25 | 0 | 14 | 2 | 1 | 1 | 14 |
| last60d | 2026-07-26 | 2 | 32 | 5 | 3 | 3 | 39 |
| 90d | 2026-06-26 | 3 | 53 | 6 | 8 | 4 | 66 |
| last180d | 2026-03-28 | 13 | 284 | 9 | 43 | 11 | 484 |
| 360d | 2025-09-29 | 26 | 420 | 9 | 126 | 23 | 945 |
| last720d | 2024-10-04 | 36 | 490 | 9 | 208 | 27 | 1169 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [xonsh-x86_64.AppImage](https://github.com/xonsh/xonsh/releases/download/0.24.2/xonsh-x86_64.AppImage) | 27.8 MiB | `other` |
| [xonsh-x86_64.AppImage.sha256](https://github.com/xonsh/xonsh/releases/download/0.24.2/xonsh-x86_64.AppImage.sha256) | 88 B | `other` |

## 改进这些数据

xonsh 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `xonsh` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/xonsh.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260924.yml` · 2026-09-24T04:57:38Z._
